...
class ClaudeOpsConfig:
    ...
    # Session Reconnection Settings (v2.1)
    @property
    def session_reconnect_max_duration(self) -> int:
        """Maximum time to retry reconnecting (seconds)"""
        return int(os.getenv("SESSION_RECONNECT_MAX_DURATION", "300"))

    @property
    def session_reconnect_initial_backoff(self) -> int:
        """Initial backoff delay for reconnection (seconds)"""
        return int(os.getenv("SESSION_RECONNECT_INITIAL_BACKOFF", "1"))

    @property
    def session_reconnect_max_backoff(self) -> int:
        """Maximum backoff delay for reconnection (seconds)"""
        return int(os.getenv("SESSION_RECONNECT_MAX_BACKOFF", "30"))

    # Telegram Rate Limiting (v2.1)
    @property
    def telegram_rate_limit_enabled(self) -> bool:
        """Enable custom rate limit handling"""
        return os.getenv("TELEGRAM_RATE_LIMIT_ENABLED", "true").lower() == "true"

    @property
    def telegram_backoff_initial(self) -> int:
        """Initial backoff for rate limit retries (seconds)"""
        return int(os.getenv("TELEGRAM_BACKOFF_INITIAL", "1"))

    @property
    def telegram_backoff_max(self) -> int:
        """Maximum backoff for rate limit retries (seconds)"""
        return int(os.getenv("TELEGRAM_BACKOFF_MAX", "60"))

    # Dangerous Command Confirmation (v2.1)
    @property
    def command_confirmation_timeout(self) -> int:
        """Timeout for dangerous command confirmations (seconds)"""
        return int(os.getenv("COMMAND_CONFIRMATION_TIMEOUT", "60"))

    # Screen History Depth (v2.1)
    @property
    def session_screen_history_lines(self) -> int:
