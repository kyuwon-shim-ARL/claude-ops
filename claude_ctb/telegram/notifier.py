...
class SmartNotifier:
    ...
    def __init__(self, config: Optional[ClaudeOpsConfig] = None):
        ...
        self._message_queue = get_global_queue()  # T031: Message queue for rate limiting
    ...
    def _send_telegram_notification(self, message: str) -> bool:
        """
        Send notification via Telegram with rate limit handling.
        T031: Wraps requests with exponential backoff queue retry logic.
        """
        try:
            try:
                import requests
            ...
            # T031: Handle rate limit errors
            if response.status_code == 429:  # Too Many Requests
                if rate_limit_enabled:
                    # Extract retry_after from response if available
                    retry_after = response.json().get('parameters', {}).get('retry_after', None)
                    logger.warning(
                        f"Rate limit hit (429). Enqueuing message for retry. "
                        f"Retry after: {retry_after}s" if retry_after else "Rate limit hit (429). Enqueuing message."
                    )

                    # Enqueue message with HIGH priority for retry
                    ...
                    return False  # Failed this time, but queued for retry
                else:
...
💡 **답장하려면** 이 메시지에 Reply로 응답하세요!"""
    ...
    def send_work_completion_notification(self) -> bool:
        ...
        # Get actual working directory from tmux session (not claude-ops dir)
        try:
            result = subprocess.run(
                f"tmux display-message -p -t {session_name} -F '#{{pane_current_path}}'",
                shell=True,
                capture_output=True,
                text=True,
                timeout=2
...
{task_completion.details}"""
    ...
    def extract_work_context(self) -> str:
        ...
        try:
            ...
            # Return content directly (no duplicate headers - already in message)
            return content
