...
class SessionStateAnalyzer:
    ...
    def _detect_working_state(self, screen_content: str) -> bool:
        ...
        # Gate: a `· done <time>` completion line owns whatever trails it, so
        # "still running" after `done` is NOT working (see _bg_tail_is_done).
        ...
        for line in recent_lines:
            ...
            if m:
                ...
                # A `· done` stamp on that line settles it either way.
                _bg_tail_live = (
                    _last_before_prompt is not None
                    and ('background task' in _last_before_prompt
                         or 'local agents' in _last_before_prompt)
                    and not self._DONE_MARKER_RE.search(_last_before_prompt)
                )
                _guard_c_direct = (
                    _last_before_prompt
                    and _past_completion_re.search(_last_before_prompt)
                    and not _bg_tail_live
                )
                _guard_c_fallback = (
        ...
        # The three background-tail patterns yield to a `· done` stamp.
        _bg_tail_patterns = {
            "background task still running", "local agents still running", "to manage)",
        }
        # Judged on the last few non-blank lines, not just check_lines: a wrapped
        # tail puts `done` one line above the fragment that check_lines holds.
